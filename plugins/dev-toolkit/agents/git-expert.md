...
1. Read `.claude/dev-toolkit.md` for:
...
2. Read `.claude/dev-toolkit.local.md` for:
...
Branch names MUST follow the pattern from `.claude/dev-toolkit.md`:

**Default pattern:** `{username}/{ticket_id}/{type}/{description}`

**Pattern variables:**
- `{username}` - From `.claude/dev-toolkit.local.md`
