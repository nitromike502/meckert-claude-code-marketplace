...
```bash
/dev-toolkit:dev-strategy approved   # For complex features
/dev-toolkit:dev-strategy rapid      # For straightforward changes
/dev-toolkit:dev-strategy parallel   # For independent tasks
```
...
**Slash Command:**
`.claude/commands/dev-toolkit:dev-strategy.md`
...
3. **Execute the command:** Run `/dev-toolkit:dev-strategy <type>` to set the strategy
...
**Commands:**
- `/dev-toolkit:project-status` - Intelligent ticket selection
- `/dev-toolkit:swarm <ticket-id>` - Execute SWARM workflow
...
- **/dev-toolkit:dev-strategy command:** Interactive strategy selection tool
