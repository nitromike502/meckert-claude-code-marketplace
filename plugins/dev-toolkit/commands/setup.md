...
You are guiding the user through setting up dev-toolkit for their project. Create lean configuration files and generate useful templates.
...
1. **`.claude/dev-toolkit.md`** - Shared project config (committed to repo)
   - Maintained by tech lead / team
   - Contains: tech stack, ticketing config, git conventions, template references
   - Should be version controlled

2. **`.claude/dev-toolkit.local.md`** - Personal user config (gitignored)
...
2. **Next steps:**
   - Add `.claude/dev-toolkit.local.md` to `.gitignore`
   - Review and customize templates in `.claude/templates/`
   - Commit `.claude/dev-toolkit.md` and `.claude/templates/`
   - Run `/dev-toolkit:help` to see available commands
...
- **Suggest .gitignore** - Add `.claude/dev-toolkit.local.md`
